...
namespace MusicStoreServer.Web.Controllers.ApiControllers.V1
{
    ...
    public class AccountController : CustomApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel coach)
        {
            ...
            {
                bool recaptchaResult = ValidRecaptcha(coach.Captcha);
        ...
        public async Task<IHttpActionResult> ResetPassword(ResetPasswordBindingModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    var resultReset = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Password);

                    if (!resultReset.Succeeded)
                    {
                        result.Error.Code = ErrorStatusCode.InvalidToken;
                        result.Error.Description = resultReset?.Errors?.FirstOrDefault();
                    }
                }
            }
            else
